...
Dear ImGui is highly portable and only requires a few things to run and render, typically:
 ...
 - Required: creating, updating and destroying textures_.
...
Otherwise, rendering will be limited to 64K vertices_ per window, which may be limiting for advanced plot.
As an alternative, you may also use `#define ImDrawIdx unsigned int` in your `imconfig.h` file to support 32-bit indices_.

```cpp
void MyImGuiBackend_RenderDrawData(ImDrawData* draw_data)
{
    // TODO: Update textures_.
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // By default the indices_ ImDrawIdx are 16-bit, you can change them to 32-bit in imconfig.h if your engine doesn't support 16-bit indices_.
...
- During rendering, e.g. in your RenderDrawData function, iterate `ImDrawData->Textures` array and process all textures_.
- During shutdown, iterate the `ImGui::GetPlatformIO().Textures` and destroy all textures_.
...
void MyImGuiBackend_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        // - We only ever write to textures_ regions which have never been used before!
